classdef FFNNS < handle
    ...
    methods % constructor, evaluation, sampling, print methods
        ...
        % added by Yanbing
        % Compute the gradient of a FFNN symbolically
        function fPrime = symbolicGradient(obj,y_sym)
            fPrime = eye(obj.nI);
            for i=1:obj.nL
                fPrime = obj.Layers(i).symbolicGradient(y_sym) * fPrime;
                y_sym = obj.Layers(i).evaluate(y_sym);
            end
        end
