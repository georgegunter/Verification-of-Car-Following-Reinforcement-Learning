...
val basicOptions : opt list =
  [{switches = ["--"], arguments = [],
   ...
   {switches = ["-t", "--verbose"], arguments = ["0..5"], (*LCP: tracing*)
    description = "the degree of verbosity",
    processor = intOpt (SOME 0, SOME 5) endOpt (fn i => traceLevel := i)},
...
fun usageInformation ({header,footer,options,...} : allOptions) =
  let
