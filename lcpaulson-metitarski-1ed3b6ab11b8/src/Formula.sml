...
fun freeSkos False = NameSet.empty
  | freeSkos True = NameSet.empty
  | freeSkos (Atom (p,args)) = Term.freeSkosList args
  | freeSkos (Not p) = freeSkos p
  | freeSkos (And(p,q)) = NameSet.union (freeSkos p) (freeSkos q)
  | freeSkos (Or(p,q)) = NameSet.union (freeSkos p) (freeSkos q)
  | freeSkos fm = raise Error "Formula.freeSkos";

local
  fun bv vs [] = vs
    | bv vs (True :: fms) = bv vs fms
    | bv vs (False :: fms) = bv vs fms
    | bv vs (Atom atm :: fms) = bv vs fms
    | bv vs (Not p :: fms) = bv vs (p :: fms)
    | bv vs (And (p,q) :: fms) = bv vs (p :: q :: fms)
    | bv vs (Or (p,q) :: fms) = bv vs (p :: q :: fms)
    | bv vs (Imp (p,q) :: fms) = bv vs (p :: q :: fms)
    | bv vs (Iff (p,q) :: fms) = bv vs (p :: q :: fms)
    | bv vs (Forall (v,p) :: fms) = bv (NameSet.add vs v) (p :: fms)
    | bv vs (Exists (v,p) :: fms) = bv (NameSet.add vs v) (p :: fms);

  fun add (fm,vs) = bv vs [fm];
in
  fun boundVars fm = add (fm,NameSet.empty);
end;
  ...
  fun promote (Term.Var v) = Atom (v,[])
    | promote (Term.Fn (f,tms)) =
      if Name.equal f truthName andalso null tms then
        ...
        Atom (f,tms)
    (* added to remove Matches not exhaustive warning *)
   | promote _ = raise Bug "Formula.promote in Formula.fromString";
in
