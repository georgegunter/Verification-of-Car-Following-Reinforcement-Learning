...
fun nodePriorityOrder node1_node2 =
    let
...
fun treeAppend tree1 tree2 =
    ...
    | T node1 =>
      ...
      | T node2 =>
        ...
        in
          case nodePriorityOrder node1_node2 of
            LESS =>
            let
          ...
          | EQUAL => raise Bug "KeyMap.treeAppend: equal keys"
          | GREATER =>
            let
      ...
      fun ins left_right path =
          ...
          | step :: rest =>
            let
              ...
              val node_n = (node,n)
            in
              case nodePriorityOrder node_n of
                LESS =>
                let
                  val (left,right) = left_right
              ...
              | EQUAL => raise Bug "KeyMap.insertNodePath: equal keys"
              | GREATER =>
                let
...
val insertNodePath = fn node => fn path =>
    let
      val result = insertNodePath node path
      and result' = insertNodePath' node path
...
(*BasicDebug
val nodePartition = fn pkey => fn node =>
    let
      val result as (l,kvo,r) = nodePartition pkey node
