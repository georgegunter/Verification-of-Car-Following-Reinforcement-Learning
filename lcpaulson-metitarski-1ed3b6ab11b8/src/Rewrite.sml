...
(* Useful functions for getting at internals *)

fun getKnown (Rewrite {known,...} ) = known;

fun getOrder (Rewrite {order,...} ) = order;
...
fun termReducible order known id =
    let
      ...
      and subtermRed (Term.Var _) = false
        ...
        (* added to remove Matches are not exhaustive warning *)
        | subtermRed _ = raise Bug "bad subterm in subtermRed in Rewrite.sml"
    in
      termRed
    end;
