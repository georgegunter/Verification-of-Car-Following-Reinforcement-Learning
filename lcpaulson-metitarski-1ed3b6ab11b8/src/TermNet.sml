    ...
    | cmp (q1_q2 :: qs) =
      if Portable.pointerEqual q1_q2 then cmp qs
        ...
        case q1_q2 of
          (Var,Var) => EQUAL
        | (Var, _) => LESS
        | (Rat r1, Rat r2) => Rat.compare (r1,r2)
        | (Rat _, Fn _) => LESS
        | (Fn f1, Fn f2) => fnCmp f1 f2 qs
        | _ => GREATER  (* (Rat _, Var _), (Fn _, Var _), (Fn _, Rat _) *)
...
local
  fun qm [] = true
    | qm ((Var,_) :: rest) = qm rest
    | qm ((Rat q, Term.Rat r) :: rest) = Rat.eq (q,r) andalso qm rest
...
local
  fun qn qsub [] = SOME qsub
    ...
    | qn qsub ((Term.Rat q, Rat r) :: rest) =
      if Rat.eq (q,r) then qn qsub rest
      else NONE
...
local
  fun qv Var x = x
    | qv x Var = x
    | qv (Rat q) (Rat r) = if Rat.eq (q,r) then Rat q else raise Error "TermNet.qv"
    | qv (Fn (f,a)) (Fn (g,b)) = if NameArity.equal f g then Fn (f, zipWith qv a b) 
                                 else raise Error "TermNet.qv"
    | qv _ _ = raise Error "TermNet.qv"
  ...
  fun qu qsub [] = qsub
    ...
    | qu qsub ((Rat q, Term.Rat r) :: rest) =
      if Rat.eq (q,r) then qu qsub rest
    ...
    | qu qsub (_ :: rest) = raise Error "TermNet.qu";  (*Differing constructors*)
in
...
(*Rationals as dummy functions*)
fun ratArity r = (Rat.toString r, 0)   (*much better than just "" *)

fun qdestRF Var = raise Bug "qdestRF"
  | qdestRF (Rat r) = (ratArity r, [])
  | qdestRF (Fn fl) = fl;

fun destRF (Term.Var _) = raise Bug "destRF"
  | destRF (Term.Rat r) = (Rat.toString r, [])
  | destRF (Term.Fn fl) = fl;
...
fun foldEqualTerms pat inc acc =
    let
      fun fold ([],net) = inc (pat,net,acc)
        ...
        | fold (pat :: pats, Multiple (_,fns)) =
          let val (f,a) = qdestRF pat
          in case NameArityMap.peek fns f of
             NONE => acc
...
local
  fun fold _ acc [] = acc
    ...
    | fold
        inc acc
        ((pat :: pats, stack, Multiple (v,fns)) :: rest) =
      let
        val (f,a) = qdestRF pat
        val rest =
            case v of
      ...
      let
        ...
        val rest =
            case tm of
              Term.Var _ => rest
            | Term.Rat r =>
              (case NameArityMap.peek fs (ratArity r) of
                NONE => rest
              | SOME n => (n, tms) :: rest)
            | Term.Fn (f,l) =>
...
local
  ...
  fun mat acc [] = acc
    ...
    | mat acc ((qsub, Multiple (_,fns), tm :: tms) :: rest) =
      let
        val (f,a) = destRF tm
        val rest =
...
local
  ...
  fun mat acc [] = acc
    ...
    | mat acc ((qsub, Multiple (v,fns), tm :: tms) :: rest) =
      let
        val (f,a) = destRF tm
