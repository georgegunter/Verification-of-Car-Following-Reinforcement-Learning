...
local
  fun inv (v, Term.Var w, s) =
    ...
    | inv (_, _, _) = raise Error "Subst.invert: non-variable";
in
...
local
  fun matchList sub [] = sub
    ...
    | matchList sub ((Term.Rat q, Term.Rat r) :: rest) =
      if Rat.eq (q,r) then matchList sub rest
...
local
  ...
  and solve' sub (Term.Var v) tm rest =
    ...
    | solve' sub (Term.Rat q) (Term.Rat r) rest =
      if Rat.eq (q,r) then solve sub rest
      else
    ...
    | solve' sub (Term.Fn (f1,args1)) (Term.Fn (f2,args2)) rest =
      if Name.equal f1 f2 andalso length args1 = length args2 then
        ...
        raise Error "Subst.unify: different structure"
    | solve' sub _ _ rest = raise Error "Subst.unify: different structure";
in
