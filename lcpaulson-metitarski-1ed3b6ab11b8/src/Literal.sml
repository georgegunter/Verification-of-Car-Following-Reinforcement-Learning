...
fun isGround (pol, (a,ts)) = List.all Term.isGround ts;

fun containsSkolemFunction (_,(_,ts)) = List.exists Term.containsSkolemFunction ts;
...
fun mkBinop (pol,rel) (a,b) : literal = (pol, Atom.mkBinop rel (a,b));
...
fun freeSkos lit = Atom.freeSkos (atom lit);
...
fun is_label ((_, (rel,ts)) : literal) = null ts andalso String.isPrefix "_" rel;

val atom_weight_r = ref 500;

(*LCP: Special status for equality*)
fun typedSymbols (true, atm as ("=",_)) = 1000 + Atom.typedSymbols atm
  | typedSymbols (pol, atm) =
      if is_label (pol, atm)
      then if pol then !atom_weight_r else 0 (*LCP: for case splitting*)
      else Atom.typedSymbols atm;
...
fun hash (true, atm)  = Atom.hashw (atm, 0w0)
  | hash (false, atm) = Atom.hashw (atm, 0w1);
...
struct
  ...
  val freeSkos =
      let
        fun f (lit,set) = NameSet.union set (Literal.freeSkos lit)
      in
        foldl f NameSet.empty
      end;
  ...
  val hash = foldl (fn (lit,w) => Word.xorb (Literal.hash lit, w)) 0w0;
